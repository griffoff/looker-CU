view: ipm_campaign_to_outcome {
  ...
  derived_table: {
    sql:
     ...
     INNER JOIN (
          SELECT c.message_id, c.message_version_no, e.user_sso_guid, min(e.event_time) AS first_event_time, MAX(e.event_time) AS last_event_time, MIN(CASE WHEN e.event_action IN ('CLICKED', 'DISMISSED') THEN e.event_time END) AS clicked_time
