explore: spring_review_locker_vs_expired_cu_usage_fall_2019 {}

view: spring_review_locker_vs_expired_cu_usage_fall_2019 {
  derived_table: {
    sql: WITH
        ...
        ,locker_activations_and_dashboard_clicks AS
        (
          SELECT
          ...
          LEFT JOIN prod.cu_user_analysis.all_events ae
              ...
              AND ae.event_time >= e.effective_from
              AND ae.event_time <= e.effective_to
          WHERE subscription_state_current = 'read_only'
          GROUP BY 1
          )
          ,expired_activations_and_dashboard_clicks AS
          (
          SELECT
              e.user_sso_guid_merged
              ,COUNT(DISTINCT CASE WHEN actv_dt <= '2018-12-15' THEN actv_isbn END) AS activations_on_or_prior_20181215
          ...
          LEFT JOIN prod.cu_user_analysis.all_events ae
              ...
              AND ae.event_time >= e.effective_from
              AND ae.event_time <= e.effective_to
          WHERE e.subscription_state_current = 'provisional_locker'
          GROUP BY 1
          )
          ,locker_and_expired_activations_and_dashboard_use AS
          (
          SELECT
              'Locker user' AS locker_vs_expired
              ,*
          FROM locker_activations_and_dashboard_clicks
          UNION
          SELECT
              'Expired user' AS locker_vs_expired
               ,*
          FROM expired_activations_and_dashboard_clicks
          )
          SELECT
            *
          FROM locker_and_expired_activations_and_dashboard_use
 ...
 ;;
  ...
  dimension: locker_vs_expired {
    type: string
    sql: ${TABLE}."LOCKER_VS_EXPIRED" ;;
  }
  ...
  measure: activations_on_or_prior_20181215_avg {
    label: "Average # of activatons on or prior to Dec 15, 2018"
    type: average
    sql: ${TABLE}."ACTIVATIONS_ON_OR_PRIOR_20181215" ;;
  }
  ...
  measure: activations_after_20181215_avg {
    label: "Average # of activatons after Dec 15, 2018"
    type: average
    sql: ${TABLE}."ACTIVATIONS_AFTER_20181215" ;;
  }
  ...
  measure: browse_catalogue_clicks_avg {
    label: "Average # of browse catagloue clicks"
    type: average
    sql: ${TABLE}."BROWSE_CATALOGUE_CLICKS" ;;
  }
  ...
  set: detail {
    fields: [
      ...
      searches_count,
      added_content_to_dashboard,
      browse_catalogue_clicks,
      courseware_launches,
      ebook_launches
    ]
  }
